...
namespace ContatosApp.Infra.Data.Repositories
{
    public class UsuarioRepository : IUsuarioRepository
    {
        ...
        public Usuario? Get(string email)
        {
            using (var dataContext = new DataContext())
            {
                ...
                var query = @"SELECT * FROM USUARIO 
                    WHERE EMAIL = @Email";
                return dataContext.Database.GetDbConnection().Query<Usuario>(query, new
        ...
        public Usuario? Get(string email, string senha)
        {
            using (var dataContext = new DataContext())
            {
                /* return dataContext.Set<Usuario>().Where(u=>u.Email.Equals(email) && u.Senha.Equals(senha)).FirstOrDefault();*/
